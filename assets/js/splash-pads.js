...
console.log('splash pads filter works');
...
function filterData(data, selectedFeatures) {
    // Always filter to splash pads only first
    const splashPadData = data.filter(row => {
        const waterPlayValue = (row["Water Play"] || '').toLowerCase();
        return waterPlayValue.includes("splash pad");
    });

    // Then apply any additional optional feature filters
    return splashPadData.filter(row => {
...
function clearAllFilters() {
    ...
    // Uncheck all the "Select Feature" checkboxes
